...
pub fn detect_folder_skills(folder_path: String) -> Result<Vec<DetectedSkill>, String> {
  ...
  // First, check if folder itself has SKILL.md (single skill folder)
...
pub fn install_zip_skill(
  ...
  // Otherwise, extract to temp and copy only specified subdirectory
  if skill_path.is_empty() {
    ...
    // Copy only specified skill subdirectory
    let source_dir = temp_dir.join(&skill_path);
    if !source_dir.exists() {
      let _ = fs::remove_dir_all(&temp_dir);
      return Err(format!("Skill path not found in ZIP: {}", skill_path));
    }
...
pub fn install_github_skill(
  ...
  let skill_path_ref = Path::new(&skill_path);
  let skill_name = skill_path_ref
    .file_name()
    .map(|n| n.to_string_lossy().to_string())
    .unwrap_or_else(|| "unnamed-skill".to_string());
...
pub fn install_folder_skill(
  ...
  let (skill_name, source_dir) = if skill_path.is_empty() {
    let folder = Path::new(&folder_path);
    let name = folder
      .file_name()
      .map(|n| n.to_string_lossy().to_string())
    ...
    let name = skill_path
      ...
      .map(|n| n.to_string_lossy().to_string())
      .unwrap_or_else(|| "unnamed-skill".to_string());
    let source = Path::new(&folder_path).join(&skill_path);
    (name, source)
  };
