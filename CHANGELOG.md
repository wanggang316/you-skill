...
- Added an Instructions library, a sidebar entry next to the Skill Library for agent instruction files (`AGENTS.md`, `CLAUDE.md`). Each entry is one Markdown file kept under `~/.youskill/instructions/<name>.md` and tracked by `.instruction-lock.json`; it can be imported from local files or a folder, from a GitHub repository, folder or file URL, or created empty, and edited in place from its Content tab (unmodified copies follow the save); the detail shows where each entry came from. The instruction files found at user level and in registered projects are imported on their own, as soon as the library is listed or a project is added: a file identical to an entry becomes one of its install locations, anything else gets an entry named `user-claude` or `<project>-agents`, and a project's instruction files link to their entries. Installing writes the file each chosen agent reads at user level or in a project (copy or symlink, agents that share a file are grouped); a file that already holds a different instruction is refused unless overwritten on purpose. Installed files get the same three-way drift as skills with push, adopt, accept and diff, and the project detail's Memory section is now called Instructions.
