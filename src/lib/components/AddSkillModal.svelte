<script>
  import { X, FileArchive, Github, Check, AlertCircle, Loader2, Folder } from '@lucide/svelte'
  import { t } from '../i18n'
  import { detectZipSkills, detectGithubSkills, installZipSkill, installGithubSkill, detectFolderSkills, installFolderSkill } from '../api/skills'
  ...
  async function handleSelectZipFile() {
    try {
      ...
      const result = await open({
        ...
        directory: false,
        filters: [
          { name: 'Skill Files', extensions: ['skill', 'zip'] }
        ]
      })
      if (result) {
                ...
                <div class="max-h-48 space-y-2 overflow-y-auto rounded-xl border border-[var(--base-300)] bg-[var(--base-200)] p-2">
                  {#each detectedFolderSkills as skill}
                    ...
                    >
                      <div>
                        <p class="font-medium">{skill.name}</p>
                        <p class={`text-xs ${selectedFolderSkill?.path === skill.path ? 'text-[var(--primary-content)] opacity-80' : 'text-[var(--base-content-muted)]'}`}>
                          {skill.path}
                        </p>
                      </div>
